...
const PuzzleGame = () => {
  ...
  // Floating 효과를 위한 애니메이션 시간
  const floatingTimeRef = useRef(0);
  ...
  const renderPuzzle = () => {
    ...
    groupsRef.current.forEach((group) => {
      ...
      group.pieces.forEach((piece) => {
        ...
        let wx = renderX + piece.relativePos.x * scale;
        let wy = renderY + piece.relativePos.y * scale;

        // 🌌 보관소에 있는 조각에 무중력 floating 효과
        if (isInTrayArea && group !== draggedGroupRef.current) {
          // 각 조각마다 고유한 phase (위치 기반)
          const phase = (piece.gridX * 0.5 + piece.gridY * 0.7) * Math.PI;
          // 시간에 따라 위아래로 움직임 (진폭 8px, 주기 2.5초)
          const floatY = Math.sin(floatingTimeRef.current * 0.0025 + phase) * 8;
          // 좌우로도 살짝 움직임 (진폭 4px, 다른 주기)
          const floatX = Math.cos(floatingTimeRef.current * 0.002 + phase * 0.7) * 4;
          wx += floatX;
          wy += floatY;
        }
  ...
  const handleMouseMove = (e) => {
    ...
    if (deltaTime > 0 && draggedGroupRef.current) {
      const deltaX = mouseX - lastMousePosRef.current.x;
      const deltaY = mouseY - lastMousePosRef.current.y;
      
      // 속도 = 거리 / 시간, 우주 느낌을 위해 증폭
      draggedGroupRef.current.velocity = {
        x: (deltaX / deltaTime) * 50, // 더 강한 관성
  ...
  const handleMouseUp = () => {
    ...
    if (draggedGroupRef.current) {
      ...
      // 🌌 관성 효과: 드래그를 놓아도 velocity는 유지됨 (아래에서 감속)
      const speed = Math.sqrt(activeGroup.velocity.x ** 2 + activeGroup.velocity.y ** 2);
      if (speed > 1) {
        console.log('🚀 관성 발동! 속도:', speed.toFixed(2), 'px/frame');
      }
      ...
      while (mergedAny) {
        ...
        for (const targetGroup of groupsRef.current) {
          ...
          if (PuzzleEngine.tryMerge(activeGroup, targetGroup)) {
            ...
            activeGroup.velocity = { x: 0, y: 0 }; // 병합되면 관성 초기화
            mergedAny = true;
      ...
      if (activeGroup && !activeGroup.isLocked) {
        ...
        if (snapPos && snapPos.canSnap) {
          ...
          activeGroup.lock();
          activeGroup.velocity = { x: 0, y: 0 }; // 고정되면 관성 멈춤
  ...
  // 렌더링 루프 (드래그 중 부드러운 업데이트 + floating 효과 + 관성 효과)
  useEffect(() => {
    if (!isPaused && isLoadedRef.current) {
      const interval = setInterval(() => {
        floatingTimeRef.current += 16; // 시간 증가
        
        // 🌌 관성 효과: 모든 그룹에 velocity 적용 및 감속
        groupsRef.current.forEach((group) => {
          if (group.isLocked) return; // 고정된 그룹은 움직이지 않음
          if (group === draggedGroupRef.current) return; // 드래그 중인 그룹은 마우스로 제어
          
          // velocity가 있으면 위치 업데이트
          const speed = Math.sqrt(group.velocity.x ** 2 + group.velocity.y ** 2);
          
          if (speed > 0.3) { // 최소 속도 (낮춰서 더 오래 움직임)
            group.position.x += group.velocity.x;
            group.position.y += group.velocity.y;
            
            // 감속 (마찰력) - 우주니까 천천히 감속
            const damping = 0.96; // 96% 유지 = 4% 감속 (더 오래 미끄러짐)
            group.velocity.x *= damping;
            group.velocity.y *= damping;
          } else {
            // 너무 느리면 완전히 멈춤
            group.velocity.x = 0;
            group.velocity.y = 0;
