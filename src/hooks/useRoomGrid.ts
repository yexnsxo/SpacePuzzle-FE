...
import { TileType, GridCell, Item, GRID_BASE_SIZE, PlacedItem } from '../types/roomBuilder';

const SAVE_KEY = 'spacepuzzle-room-grid';

export const useGrid = (gridWidth: number = GRID_BASE_SIZE, gridHeight: number = GRID_BASE_SIZE) => {
  // 초기 그리드 생성
  ...
  const [grid, setGrid] = useState<GridCell[][]>(() => {
    ...
    if (saved) {
      try {
        ...
        // 그리드 크기가 변경되었는지 확인
        if (parsed.length !== gridHeight || (parsed[0] && parsed[0].length !== gridWidth)) {
          console.log(`🔄 그리드 크기 변경 감지: ${parsed[0]?.length || 0}x${parsed.length} → ${gridWidth}x${gridHeight}`);
  ...
  const isWallConnectedToFloor = (x: number, y: number): boolean => {
    ...
    for (let checkY = y + 1; checkY < gridHeight; checkY++) {
  ...
  const canPlaceWall = (x: number, y: number): boolean => {
    ...
    if (y + 1 >= gridHeight) return false;
          ...
          if (originItems.length > 0) {
            ...
            for (let dy = 0; dy < sizeY; dy++) {
              for (let dx = 0; dx < sizeX; dx++) {
                ...
                if (targetY < gridHeight && targetX < gridWidth) {
  ...
  const updateWallColor = (x: number, y: number, color: string | null) => {
    setGrid((prevGrid) => {
      ...
      while (stack.length > 0) {
        ...
        if (current.x < gridWidth - 1 && !needsRightBorder(current.x, current.y)) {
          stack.push({x: current.x + 1, y: current.y});
        }
      }
  ...
  return { 
    ...
    canPlaceItem, 
    resetGrid,
    baseSize: { width: gridWidth, height: gridHeight },
    updateWallColor,
    updateFloorColor,
  };
};
