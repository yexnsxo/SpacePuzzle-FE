...
import { TileType, TILE_SIZE, Item } from '../types';
import './Grid.css';
...
const Grid = () => {
  const { grid, updateTile, placeItem, canPlaceItem, resetGrid, baseSize, updateWallColor, updateFloorColor } = useGrid();
  ...
  const isWallDirectlyOnFloor = (x: number, y: number): boolean => {
    ...
    if (y + 1 < baseSize && grid[y + 1][x].tileType === 'FLOOR') {
      return true;
    }
    return false;
  };
  ...
  return (
    <div className="builder-container">
      ...
      {/* 색상 팔레트 (벽지/바닥재 모드일 때만 표시) */}
      {(currentMode === 'WALLPAPER' || currentMode === 'FLOORING') && (
        <div style={{
          display: 'flex',
          gap: '10px',
          padding: '15px',
      ...
      <div 
        ...
        style={{
          display: 'grid',
          gridTemplateColumns: `repeat(${baseSize}, ${TILE_SIZE}px)`,
          gridTemplateRows: `repeat(${baseSize}, ${TILE_SIZE}px)`,
          position: 'relative',
          userSelect: 'none',
